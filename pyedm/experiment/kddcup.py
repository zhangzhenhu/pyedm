...
def init_option():
    ...
    parser.add_argument("-m", "--model", dest="model", choices=['c', 'e'], default='e',
                        help=u"c:Challenge e:experiment")
    return parser
...
def extend_predict(df_train, df_test):
    ...
    for index, row in tqdm(df_test.iterrows(), total=df_test.shape[0]):
        ...
        rmse_u += (answer - user_acc if user_acc is not None else 0.5) ** 2
        rmse_i += (answer - item_acc if item_acc is not None else 0.5) ** 2
...
def run_irt(df_train: pd.DataFrame, df_test: pd.DataFrame, df_item_info: pd.DataFrame):
    from pyedm.model.irt.uirt import UIRT

    df_item_info['b'] = -df_item_info['difficulty']

    df_train.drop_duplicates(subset=['knowledge', 'user', 'item_name'], inplace=True)

    df_train['trace_name'] = df_train['knowledge']  # + '_' + df_train['user']
    df_train['item_id'] = df_train['item_name']
    df_train['user_id'] = df_train['user']
    df_train = df_train.join(df_item_info[['b']], on='item_name', how='left')

    df_test['trace_name'] = df_test['knowledge']  # + '_' + df_train['user']
    df_test['item_id'] = df_test['item_name']
    df_test['user_id'] = df_test['user']
    df_test['b'] = df_test.join(df_item_info[['b']], on='item_name', how='left')['b']

    df_train_g = df_train.groupby('trace_name')
    models = {}
    for trace_name, trace_train in df_train_g:
        # print(trace_name)
        trace_train = trace_train.sort_values(['user', 'item_id'])
        model = UIRT(model="1PL")
        # model.set_abc(df_item_info, columns=['b'])
        model.fit(trace_train)
        model.estimate_theta()
        # print(model.user_vector)
        # key = (trace_name,)
        models[trace_name] = model.user_vector
        # break
    df_test['pred_prob'] = np.nan
    # n_i = 0
    n_k = 0
    ...
    for index, row in tqdm(df_test.iterrows(), total=df_test.shape[0]):
        item_b = row['b']
        if np.isnan(item_b):  # 空知识点暂时不处理
            # predict_list.append(np.nan)
            n_item += 1
            continue
        trace_name = row['trace_name']
        user_vector = models[trace_name]
        try:
            theta = user_vector.loc[row['user_id']]['theta']
        except:
            n_model_not += 1
            # print(row)
            continue

        prob = UIRT.uirt(theta=theta, a=1.0, b=item_b, c=0.0)
        if np.isnan(prob):
            n_model_fail += 1
            # print(prob)
        df_test.loc[index, 'pred_prob'] = prob

    df_eva = df_test.loc[~df_test['pred_prob'].isna()].copy()
    print("irt: 全部测试集:%d 召回数量:%d 召回比例:%.4f n_k:%d n_item:%d n_model_not:%d n_model_fail:%d" % (
        df_test.shape[0], df_eva.shape[0], float(df_eva.shape[0]) / df_test.shape[0],
        n_k,
        n_item,
        n_model_not,
...
def load_tal_data():
    ...
    df_data.drop_duplicates(subset=['knowledge', 'user', 'item_name'], inplace=True)
    ...
    for key, df_u in df_g:
        n = len(df_u)
        if n < 8:
            df_train.append(df_u)
            # mask.extend([True] * n)
            continue
        df_train.append(df_u.iloc[:n - 1])
        df_test.append(df_u.iloc[n - 1])
...
def kdd_challenge():
    ...
    run_irt(df_train, df_test, df_item_info)
...
def main(options):
    ...
    for train, test, label in [
        ...
        print("=" * 20, 'irt', '=' * 20)
        metric1 = run_irt(df_train, df_test, df_item_info)
        metric1['data'] = label
        report.append(metric1)
    ...
    print("=" * 20, 'irt', '=' * 20)
    metric4 = run_irt(df_train, df_test, df_item_info)
    metric4['data'] = "智能练习数据"
    report.append(metric4)
