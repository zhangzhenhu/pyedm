...
cdef inline double _uirt(
    ...
    # z = slope *theta - intercept
    # 论文的版本
    cdef double z = slope * theta + intercept
    return guess + (1 - guess) * (1.0 / (1.0 + exp(-z)))
...
cdef inline int _u1irt(
    ...
    for i in range(user_count):
        for j in range(item_count):
            ...
            ret[i,j] = _uirt(theta[i],1.0,intercept[j],0.0)
    return 0
...
cdef inline int _u2irt(
    ...
    for i in range(user_count):
        for j in range(item_count):
            ...
            # z = slope[j] * theta[i] + intercept[j]
            # ret[i,j] =  1.0 / (1.0 + exp(-z))
            ret[i,j] = _uirt(theta[i],slope[j],intercept[j],0.0)
    return 0
...
cdef inline int _u3irt(
    ...
    for i in range(user_count):
        for j in range(item_count):
            ...
            ret[i,j] = _uirt(theta[i],slope[j],intercept[j],guess[j])
    return 0
...
cdef inline void _irt_matrix(
    ...
    cdef int j = 0
    cdef double z
    cdef int user_count=theta.shape[0]
    cdef int item_count = intercept.shape[0]
    if guess is None:
        if slope is None: # 1PL
            for i in range(user_count):
                for j in range(item_count):
                    ret[i,j] = _uirt(theta[i],1.0,intercept[j],0)
        else: # 2PL
            for i in range(user_count):
                for j in range(item_count):
                    ret[i,j] = _uirt(theta[i],slope[j],intercept[j],0)
    else:
        # 3PL
        for i in range(user_count):
            for j in range(item_count):
...
cdef inline double _log_likelihood(np.float_t[:] theta,
    ...
    for i in range(user_count):
        for j in range(item_count):
            ...
            # 论文的版本
            # z = slope[j] * theta[i] + intercept[j]
            # prob = guess[j] + (1.0 - guess[j]) * (1.0 / (1 + exp(-z)))
            prob=_uirt(theta[i],slope[j],intercept[j],guess[j])
            tmp = response[i][j]*log(prob) + (1-response[i][j])*log(1-prob)
            result +=tmp
    return result
...
cpdef float uirt(
    ...
    # z = slope * theta + intercept
    # return guess + (1 - guess) * (1.0 / (1.0 + exp(-z)))
    return _uirt(theta,slope,intercept,guess)
...
def log_likelihood(np.float_t[:,:] response,np.float_t[:] theta,
    ...
    assert response.shape[0]==theta.size and response.shape[1]==intercept.size==slope.size==guess.size
...
def u2irt_item_jac_and_hessian(np.float_t[:] theta,
                     ...
                     np.float_t[:,:] response):
    """
    二参数模型的一阶导数和二阶导数。z = slope*theta + intercept
    Parameters
    ----------
    response
    a
    ...
    cdef np.ndarray y_hat = np.zeros((n,m))
...
def  irt_matrix(
        ...
        np.float_t[:] slope,
        np.float_t[:] intercept,
        np.float_t[:] guess=None
           ):

    cdef int user_count=theta.size
    cdef int item_count = intercept.size

    ret = np.zeros((user_count,item_count),dtype=np.float64)
    # if slope is None:
    ...
    _irt_matrix(ret=ret,theta=theta,slope=slope,intercept=intercept,guess=guess)
    return ret
...
def irt_sequence(
    ...
    cdef np.ndarray data=np.zeros(shape=(n,),dtype=np.float64)
    ...
    if guess is None:
        if slope is None:
            ...
            for i in range(n):
                ...
                data_ptr[i] = _uirt(theta[i],1,intercept[i],0)
        else:
            # 2PL
            for i in range(n):
                data_ptr[i] = _uirt(theta[i],slope[i],intercept[i],0)
    else:
        # 2PL
        for i in range(n):
            data_ptr[i] = _uirt(theta[i],slope[i],intercept[i],guess[i])
