...
class Estimator:
    ...
    def predict(self):
        return _uirt_clib.irt_matrix(theta=self.theta,
                                      slope=self.a,
                                      intercept=self.b,
                                      guess=self.c)
...
class MLE(Estimator):
    ...
    def estimate_theta(self, **kwargs):
        def object_fun(x, response: np.ndarray):
            ret = -_uirt_clib.log_likelihood(response=response.astype(dtype=np.float64), theta=x, slope=self.a,
                                             intercept=self.b, guess=self.c)
            # print('theta llh=%f' % ret, " theta=%f" % x[0], end="\n")
            return ret

        def gradient(x, response):
            ret = _uirt_clib.uirt_theta_jac(response=response.astype(dtype=np.float64), theta=x, slope=self.a,
                                            intercept=self.b, guess=self.c)
...
class BockAitkinEM(EM):
    ...
    def _max_step(self):
        ...
        def object_func(x, rjk, wjk):
            ...
            # 预测值
            y_hat = _uirt_clib.irt_matrix(theta=theta, slope=a, intercept=b, guess=c)
