...
from . import _uirt_clib as uirt_clib
...
class UIRT(object):
    ...
    def fit(self, response: pd.DataFrame = None, orient="records", **kwargs):
        ...
        if 'a' not in labels:
            self.item_vector.loc[:, 'a'] = 1.0
        if 'b' not in labels:
            self.item_vector.loc[:, 'b'] = 0.0
        if 'c' not in labels:
            self.item_vector.loc[:, 'c'] = 0.0
        ...
        if 'theta' not in labels:
            self.user_vector.loc[:, 'theta'] = 0.0
    ...
    def set_abc(self, values, columns=None):
        ...
        if self.item_vector is None:
            ...
            self.item_vector = pd.DataFrame({
                ...
                'a': np.ones(self.item_count, dtype=np.float64),
                'b': np.zeros(self.item_count, dtype=np.float64),
                'c': np.zeros(self.item_count, dtype=np.float64),
    ...
    @classmethod
    def uirt(cls, theta, a=1.0, b=0, c=0):
        return uirt_clib.uirt(theta, a, b, c)
